...
private fun LazyItemScope.OverviewSection(
    ...
    SectionCard(MR.strings.label_overview_section) {
        ...
        // SY -->
        if (data.readingStreak > 0) {
            Row(
                modifier = Modifier.height(IntrinsicSize.Min),
            ) {
                val dayLabel = if (data.readingStreak == 1) {
                    stringResource(SYMR.strings.day)
                } else {
                    stringResource(SYMR.strings.days)
                }
                StatsOverviewItem(
                    title = "${data.readingStreak} $dayLabel",
                    subtitle = stringResource(SYMR.strings.reading_streak),
