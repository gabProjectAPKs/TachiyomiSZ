...
class ReaderViewModel @JvmOverloads constructor(
    ...
    private val chapterList by lazy {
        val manga = manga ?: return@lazy emptyList()
        // SY -->
    ...
    init {
        ...
        state.map { it.viewerChapters?.currChapter }
            ...
            .onEach { currentChapter ->
                ...
                chapterId = currentChapter.chapter.id ?: return@onEach
            }
    ...
    fun getChapters(): List<ReaderChapterItem> {
        ...
        return chapterList.mapNotNull { readerChapter ->
            val domainChapter = readerChapter.chapter.toDomainChapter() ?: return@mapNotNull null
            ReaderChapterItem(
                chapter = domainChapter,
                manga = m,
                isCurrent = readerChapter.chapter.id == currentChapter?.chapter?.id,
    ...
    private fun downloadNextChapters() {
        ...
        viewModelScope.launchIO {
            ...
            val nextChapterId = nextChapter.id ?: return@launchIO
            val chaptersToDownload = getNextChapters.await(manga.id, nextChapterId).run {
                if (readerPreferences.skipDupe().get()) {
    ...
    private fun cancelQueuedDownloads(currentChapter: ReaderChapter): Download? {
        val queuedId = currentChapter.chapter.id ?: return null
        return downloadManager.getQueuedDownloadOrNull(queuedId.toLong())?.also {
    ...
    // SY -->
    fun openQuickSettingsDialog() {
        mutableState.update { it.copy(dialog = Dialog.QuickSettings) }
    }
    // SY <--
        ...
        viewModelScope.launchNonCancellable {
            try {
                val stream = page.stream
                    ?: throw Exception("Page stream not available")
    ...
    private fun saveImages(
        ...
        val stream1 = page1.stream
            ?: throw Exception("Page 1 stream not available")
        ImageUtil.findImageType(stream1) ?: throw Exception("Not an image")
        val stream2 = page2.stream
            ?: throw Exception("Page 2 stream not available")
        ImageUtil.findImageType(stream2) ?: throw Exception("Not an image")
        val decoder1 = ImageDecoder.newInstance(stream1())
            ?: throw Exception("Failed to create image decoder for page 1")
        val decoder2 = ImageDecoder.newInstance(stream2())
            ?: throw Exception("Failed to create image decoder for page 2")
        val imageBitmap = decoder1.decode()
            ?: throw Exception("Failed to decode page 1 image")
        val imageBitmap2 = decoder2.decode()
            ?: throw Exception("Failed to decode page 2 image")
    ...
    data object QuickSettings : Dialog
    // SY <--
    }
