...
class TachiyomiImageDecoder(private val resources: ImageSource, private val options: Options) : Decoder {
    ...
    override suspend fun decode(): DecodeResult {
        ...
        val sourcePeeked = resources.sourceOrNull()?.peek()
        if (sourcePeeked != null && sourcePeeked.use { CbzCrypto.detectCoverImageArchive(it.inputStream()) }) {
            val sourcePeeked2 = resources.sourceOrNull()?.peek()
            if (sourcePeeked2 != null && sourcePeeked2.use { ImageUtil.findImageType(it.inputStream()) == null }) {
                val file = resources.fileOrNull()?.toFile()
                if (file != null) {
                    coverStream = UniFile.fromFile(file)
