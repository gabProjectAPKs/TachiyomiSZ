...
class NotificationReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            ...
            // Share backup file
            ACTION_SHARE_BACKUP -> {
                val uri = intent.getParcelableExtraCompat<Uri>(EXTRA_URI) ?: return
    ...
    private fun openChapter(context: Context, mangaId: Long, chapterId: Long) {
        // Use a coroutine with a timeout instead of blocking the BroadcastReceiver
        kotlinx.coroutines.MainScope().launch {
            val manga = withContext(Dispatchers.IO) { getManga.await(mangaId) }
            val chapter = withContext(Dispatchers.IO) { getChapter.await(chapterId) }
